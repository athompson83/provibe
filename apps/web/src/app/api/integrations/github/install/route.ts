...
import { buildInstallationUrl, signIntegrationState } from '@provibe/provider-github';
...
export async function GET(request: Request) {
  ...
  const state = signIntegrationState({ userId, workspaceId: String(project.workspace_id), projectId, expiresAt: Math.floor(Date.now() / 1000) + 600 }, config.stateSecret);
  return NextResponse.redirect(buildInstallationUrl({ appSlug: config.appSlug, state }));
}
